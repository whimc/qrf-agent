...
| Guidance ("something cool") | If **Journey** is present: shows the **3 nearest** **`poi-*` / `npc-*`** destinations on the player's **current world** (Journey public waypoints with known locations, plus WorldGuard `poi-*` region centers; optional MySQL `rg_region` keys when `journey.poi-source` includes `database`). Each choice runs **`/<journey-command-root> server waypoint <name_id>`** as the player. Set **`journey.debug-log: true`** for source counts in console. Falls back to **chat** entry if none are nearby. (LLM Journey catalog still uses portal-linked worlds via `loadGuidanceDestinations`.) |
